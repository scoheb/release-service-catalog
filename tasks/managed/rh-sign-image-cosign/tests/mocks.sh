...
function skopeo() {
  echo "$@" >> $(params.dataDir)/$(params.subdirectory)/mock_skopeo_calls
  if [ "$1" = "inspect" ]; then
...
function cosign () {
  ...
  successfull_run=$(sed -n '1p' $(params.dataDir)/$(params.subdirectory)/mock_cosign_success_calls && \
    sed -i '1d' $(params.dataDir)/$(params.subdirectory)/mock_cosign_success_calls)

  if [ "$1" = "verify" ]; then
    mock_existing_sig_file=$(echo "${*: -1}" | tr "/" "-")
    echo "$@" >> $(params.dataDir)/$(params.subdirectory)/mock_cosign_verify_calls
    # if the call shouldn't end successfully, exit with error
    if [ "$successfull_run" != "1" ]; then
      return 1
    fi
    cat "$(params.dataDir)/$(params.subdirectory)/$mock_existing_sig_file"
  else
    echo "running cosign: $@"
    echo "$@" >> "$(params.dataDir)/$(params.subdirectory)/mock_cosign_sign_calls"
    # if the call shouldn't end successfully, exit with error
    if [ "$successfull_run" != "1" ]; then
      >&2 echo "- SIMULATED ERROR -"
      echo "- SIMULATED ERROR -" >> "$(params.dataDir)/$(params.subdirectory)/mock_cosign_sign_calls"
      return 1
    fi
  fi
}
