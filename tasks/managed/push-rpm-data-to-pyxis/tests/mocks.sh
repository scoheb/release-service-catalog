...
function cosign() {
  ...
  echo "$SBOM_JSON" > "$(workspaces.data.path)/$(params.subdirectory)/downloaded-sboms/${4}"
}

function upload_rpm_data() {
  echo Mock upload_rpm_data called with: $*
  echo $* >> "$(workspaces.data.path)/$(params.subdirectory)/mock_upload_rpm_data.txt"
  ...
  then
    LOCK_FILE=$(workspaces.data.path)/$(params.subdirectory)/${3}.lock
    touch $LOCK_FILE
    sleep 2
    LOCK_FILE_COUNT=$(ls $(workspaces.data.path)/$(params.subdirectory)/*.lock | wc -l)
    echo $LOCK_FILE_COUNT > $(workspaces.data.path)/$(params.subdirectory)/${3}.count
    sleep 2
    rm $LOCK_FILE
  fi
}
...
function upload_rpm_data_cyclonedx() {
  ...
  echo $* >> "$(workspaces.data.path)/$(params.subdirectory)/mock_upload_rpm_data_cyclonedx.txt"
