...
echo "🔒 Checking permissions to access Repository CRs..."
if ! kubectl auth can-i get repositories --all-namespaces &>/dev/null; then
    ...
    if ! kubectl auth can-i get repositories -n "$NAMESPACE" &>/dev/null; then
        ...
        echo "✅ User has permission to access repositories in namespace '${NAMESPACE}'"
    fi
else
    echo "✅ User has permission to access repositories cluster-wide"
fi
...
while IFS='|' read -r repo_name secret_name component_names; do
    ...
    if [[ -n "$component_names" ]]; then
        ...
        if [[ "$component_validation_failed" == "true" ]]; then
            echo "  ❌ ERROR: One or more Components do not exist."
            validation_failed=true
        else
            echo "  ✅ SUCCESS: All Components exist in namespace '${NAMESPACE}'"
        fi
    else
