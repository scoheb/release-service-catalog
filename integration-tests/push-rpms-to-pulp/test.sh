...
# Function to wait for a specific task to start in a PipelineRun
wait_for_task_to_start() {
    ...
    echo "Waiting for task '${task_name}' to start in ${plr_name}..."
    ...
    while true; do
        ...
        if [ ${elapsed} -ge ${timeout} ]; then
            echo "🔴 Timeout waiting for task ${task_name} to start"
            return 1
        fi

        # Check if the task has a TaskRun
        local tr_name
        tr_name=$(kubectl get taskrun -n "${namespace}" \
            -l "tekton.dev/pipelineRun=${plr_name},tekton.dev/pipelineTask=${task_name}" \
            -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || echo "")

        if [ -n "${tr_name}" ]; then
            echo "✅ Task '${task_name}' started: ${tr_name}"
            return 0
        fi
...
# Function to wait for a specific task to complete in a PipelineRun
wait_for_task_to_complete() {
    ...
    echo "Waiting for task '${task_name}' to complete in ${plr_name}..."
    ...
    while true; do
        ...
        if [ ${elapsed} -ge ${timeout} ]; then
            echo "🔴 Timeout waiting for task ${task_name} to complete"
            return 1
        fi

        # Check task status
        local tr_status
        tr_status=$(kubectl get taskrun -n "${namespace}" \
            -l "tekton.dev/pipelineRun=${plr_name},tekton.dev/pipelineTask=${task_name}" \
            -o jsonpath='{.items[0].status.conditions[?(@.type=="Succeeded")].status}' 2>/dev/null || echo "")
