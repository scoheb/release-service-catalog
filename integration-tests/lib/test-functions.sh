...
create_kubernetes_resources() {
    ...
    # Pre-process YAML files with envsubst before kustomize runs
    # This handles variables like ${PULP_SIGNING_CONFIG} that contain multi-line YAML
    # which kustomize cannot parse as raw template syntax
    echo "Pre-processing YAML templates..."
    while IFS= read -r -d '' yaml_file; do
        envsubst < "$yaml_file" > "$yaml_file.tmp" && mv "$yaml_file.tmp" "$yaml_file"
    done < <(find "$tmpDir" -name "*.yaml" -print0)
...
# Get console URL for pipelineruns
...
get_pipelinerun_console_url() {
    ...
    console_url=$(kubectl config view --minify --output jsonpath="{.clusters[*].cluster.server}" \
        | sed 's/api/console-openshift-console.apps/g' | sed 's/:6443//g')
    console_url=${console_url%/}

    if [ -n "$console_url" ]; then
        echo "${console_url}/k8s/ns/${namespace}/tekton.dev~v1~PipelineRun/${pipelinerun_name}"
    else
        echo "N/A"
    fi
}
